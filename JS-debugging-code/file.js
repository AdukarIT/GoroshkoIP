...
function simple(x) {
    for (let i = x - 1; i > 1; i--) {
        if (!(x % i)) { //или if (x / i == ~~(x / i)) 
            str = "Введенное число - составное";
            break;
        } else
            str = "Введенное число - простое";
    }
    if (x == 1)
        str = "1 - не является простым числом";
    if (x < 1 || x % 1)
        str = "Введенное число не является натуральным";
    return str;
}
...
function month(number) {
    switch (number) {   //в таких случаях break можно не писать??
        case 12:
        case 1:
        case 2:
            return "winter";
